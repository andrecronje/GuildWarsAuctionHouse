...
function refreshall () {
	...
	$.getJSON("./js/wbtime.json", function(json) {
		var i = 0;
		var k = 0;
		var wbt = new Array();
...
//					id: json.worldboss[i].name.replace(/\s+/g, ''),
		...
		for (i = 0; i<k; i++) {
			...
			var tmp = '<div class="row table-content scale-'+wbt[i].scale+' '+wbt[i].id+'">'
				+'<div class="col-sm-4 wbframe'+donechk+'"><span class="wbid" hidden>'+wbt[i].id+'</span>'+wbt[i].name+'</div>'
				+'<div class="col-sm-2 localtime">'+wbt[i].lctime+'</div>'
				+'<div class="col-sm-2 psttime">'+wbt[i].uptime+'</div>'
				+'<div class="col-sm-3 waypoint">'+wbt[i].waypoint+'</div>'
				+'</div>';
